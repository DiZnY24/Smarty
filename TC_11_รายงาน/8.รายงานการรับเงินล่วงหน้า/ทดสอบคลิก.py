...
class WebTestCase(unittest.TestCase):
    ...
    def test_page_title(self):
        driver = self.driver
        self.assertEqual(driver.title, 'Statement reconcile report')  # ตรวจสอบว่า title ของหน้าเว็บถูกต้อง
...
# ทดสอบการคลิก Element
    ...
    def test_function(self):
        # หน้า จัดการลูกหนี้  
        self.driver.get('https://msm-smarty-cms-staging.hr-impact.co/report/statement-reconcile')
...
# ค้นหาองค์ประกอบ
        # element = self.driver.find_element(By.XPATH, "//div/div/div/div/div[1]/h5")
        ...
        # if element_text == "สรุปการแจ้งหนี้รายบุคลประจำงวด":
...
# วันสิ้นสุด
        ...
        try:
                element = self.driver.find_element(By.XPATH, "/html/body/div/div/main/div/div/div/div/div[1]/div/div[2]/div/div")
                assert element.is_displayed(), "Element is not visible"  
                self.assertTrue(element.is_displayed(),'ปุ่มไม่แสดงอยู่บนหน้า')
                self.assertTrue(element.is_enabled(), "ปุ่มไม่สามารถใช้งานได้")
                element.click()
        ...
        try:
                element = self.driver.find_element(By.XPATH, "/html/body/div[2]/div[3]/div/div/div/div[2]/div/div[2]/div/div[2]/div/div[4]/button[7]")
                assert element.is_displayed(), "Element is not visible"  
                self.assertTrue(element.is_displayed(),'ปุ่มไม่แสดงอยู่บนหน้า')
                self.assertTrue(element.is_enabled(), "ปุ่มไม่สามารถใช้งานได้")
                element.click()
